...
def export_segments(export_dir):
    ...
    with open(segments_path, 'w', encoding='utf-8') as f:
        ...
        for i in range(seg_qty):
            ...
            f.write("{}|{}|{}|{}|{}|{}|{}\n".format(
                i, seg_name, hex(seg_start), hex(seg_end), seg_size, perm_str, seg.type
            ))
            segment_count += 1
...
def export_data_definitions(export_dir):
    """将所有段中的有类型数据项导出到 data_definitions.txt

    结构体名称解析需要 ida_typeinf，不可用时优雅回退。
    ...
    with open(data_path, 'w', encoding='utf-8') as f:
        ...
        f.write("# Types: b=byte w=word dw=dword qw=qword ow=oword f=float d=double str=string wstr=wstring struct align\n")
        f.write("#" + "=" * 60 + "\n\n")
                ...
                try:
                    ...
                    if ida_bytes.is_data(flags):
                        ...
                        if ida_bytes.is_byte(flags):
                            size, type_name = 1, "b"
                            value_str = "0x{:02X}".format(ida_bytes.get_byte(addr))
                        elif ida_bytes.is_word(flags):
                            size, type_name = 2, "w"
                            value_str = "0x{:04X}".format(ida_bytes.get_word(addr))
                        elif ida_bytes.is_dword(flags):
                            size, type_name = 4, "dw"
                            value_str = "0x{:08X}".format(ida_bytes.get_dword(addr))
                        elif ida_bytes.is_qword(flags):
                            size, type_name = 8, "qw"
                            value_str = "0x{:016X}".format(ida_bytes.get_qword(addr))
                        elif ida_bytes.is_oword(flags):
                            size, type_name = 16, "ow"
                            value_str = ""
                        elif ida_bytes.is_float(flags):
                            size, type_name = 4, "f"
                            try:
                                import struct as _struct
                                value_str = str(_struct.unpack('f', _struct.pack('I', ida_bytes.get_dword(addr)))[0])
                            except Exception:
                                value_str = ""
                            ...
                            try:
                                import struct as _struct
                                value_str = str(_struct.unpack('d', _struct.pack('Q', ida_bytes.get_qword(addr)))[0])
                            except Exception:
                                value_str = ""
                            ...
                            try:
                                ...
                                if tid != ida_idaapi.BADADDR and HAS_IDA_TYPEINF:
                                    ...
                                    size = type_tif.get_size() if type_tif.get_type_by_tid(tid) else ida_bytes.get_data_elsize(addr, flags)
                                else:
                                    type_name = "struct"
                                    size = ida_bytes.get_data_elsize(addr, flags)
                            except Exception:
                                type_name = "struct"
                                size = max(1, ida_bytes.get_data_elsize(addr, flags))
                            value_str = ""
                    ...
                    if ida_bytes.is_data(flags):
                        ...
                        if ptr_size > 0:
                            ...
                            if ptr_value != 0:
                                ...
                                f.write("{}|{}|{}|{}|p{}\n".format(
                                    hex(addr), hex(ptr_value),
                                    target_name, target_type, ptr_size * 8
                                ))
                                pointer_count += 1
...
def export_memory(export_dir):
    ...
    print("    Total bytes: {} ({:.2f} MB)".format(total_bytes, total_bytes / (1024 * 1024)))
...
def export_decompiled_functions(export_dir, skip_existing=True):
    ...
    print("[*] Found {} functions total, {} remaining to process".format(
...
def do_export(export_dir=None, ask_user=True, skip_auto_analysis=False, worker_count=None):
    ...
    # IDA 兼容性提示
    if not HAS_IDA_TYPEINF:
        print("[!] ida_typeinf not available - type/data/pointer exports will be skipped")
    if not HAS_IDA_NAME:
    ...
    if has_hexrays:
        ...
        print("[*] Tip: If IDA crashes, restart and the export will resume from where it left off")
...
class ExportForAIPlugin(ida_idaapi.plugin_t):
    ...
    help = ("Export decompiled functions, strings, memory, imports, exports, "
